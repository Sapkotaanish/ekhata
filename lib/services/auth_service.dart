...
class AuthService {
  static Future<List<String>> registerUser(String firstname, String lastname,
    ...
    if (response.statusCode == 200) {
      ...
      String message = data["message"]?[0];
      String success = data.containsKey('success')
          ? data['success']
              ? "true"
              : "false"
          : "false";
  ...
  static Future<List<String>> fetchLogin(String email, String password) async {
    final response = await HttpService.postReq(
        "${env.BACKEND_URL}/login/", {"email": email, "password": password},
        withToken: false);
    if (response.statusCode == 200) {
      final data = json.decode(response.body);
      print(data);
